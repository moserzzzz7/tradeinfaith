...
(async()=>{
 ...
 try {
  ...
  assert.ok(!/[+-]\d[\d.]*R/.test(statsText),'No R values below 10 trades with risk');
  assert.ok(statsText.includes('R from 10 trades with risk (2/10)'),'R coverage is shown in the per-trade card');
  assert.ok(statsText.includes('Per trade')&&statsText.includes('expectancy'),'Expectancy KPI');
  ...
  assert.equal(await run('document.querySelectorAll(".hbar-track.div>i").length'),2,'Rule-based vs deviated is drawn as diverging bars');
  assert.ok(statsText.includes('+$800'),'Difference rule-based minus deviated = 600 - -200');
  ...
  assert.ok(rText.includes('+0.5R')&&rText.includes('total +5R'),'R values appear from 10 trades with risk');
  ...
  await sleep(500);
  await run(`document.querySelectorAll('html,body,.app,.main-wrap,.main').forEach(n=>{n.dataset.css=n.style.cssText;n.style.height='auto';n.style.maxHeight='none';n.style.overflow='visible'})`);
  ...
  await run(`document.querySelectorAll('[data-css]').forEach(n=>{n.style.cssText=n.dataset.css;delete n.dataset.css})`);
