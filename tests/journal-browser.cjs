...
(async()=>{
 ...
 try {
  ...
  await run('clearLearnForm();await lessonToPlaybook(11);');
  assert.equal(await run('Journal.value("ln-body").startsWith("Wait for the confirmed close.")'),true,'A lesson prefills a playbook entry');
  ...
  await run('setField("lf-emotion","");setField("lf-from","2026-09-01");setField("lf-to","2026-09-30");onListFilterChange();');
  assert.equal(await run('document.querySelectorAll("#trades-el .trade-item").length'),2,'Period filter');
