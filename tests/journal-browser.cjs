...
(async()=>{
 ...
 try {
  ...
  const scenes=await run('JSON.stringify(buildRecapScenes(_cache,"2026-08-31","2026-09-06","").map(s=>({hold:s.hold,cls:s.cls,text:s.html.replace(/<[^>]+>/g," ")})))');
  const sc=JSON.parse(scenes);
  assert.ok(sc[1].text.includes('This week') && sc[1].text.includes('2') && sc[1].text.includes('trades taken'),'Trade count scene');
  assert.ok(sc.some(s=>s.text.includes('+$600') && s.text.includes('1 without P&L not counted')),'Net P&L counts only trades with P&L and says so');
  ...
  assert.ok(statsText.includes('Average R'),'Average R KPI');
